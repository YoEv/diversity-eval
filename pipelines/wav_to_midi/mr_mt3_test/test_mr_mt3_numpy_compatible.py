#!/usr/bin/env python3
"""
兼容numpy 1.23.5的MR-MT3 音频转MIDI测试脚本
避免直接导入可能有兼容性问题的模块，主要使用命令行调用
"""
...
def transcribe_single_audio_cmdline(audio_file, output_dir, model_path=None):
    """
    使用命令行方式调用MR-MT3转录单个音频文件
    避免直接导入可能有兼容性问题的Python模块
    """
    try:
...
def batch_transcribe(input_dir, output_dir, model_path=None):
    ...
    for i, audio_file in enumerate(audio_files, 1):
        ...
        result = transcribe_single_audio_cmdline(audio_file, output_dir, model_path)
...
def main():
    parser = argparse.ArgumentParser(description="兼容numpy 1.23.5的MR-MT3 音频转MIDI工具")
    ...
    print("🎼 兼容numpy 1.23.5的MR-MT3 音频转MIDI工具")
    print("=" * 60)
