...
def init_model_and_processor(args):
    # 对于 GPU 使用 bfloat16，CPU 用 float32
    use_cuda = (device == "cuda" and torch.cuda.is_available())
    dtype = torch.bfloat16 if use_cuda else torch.float32

    # 新增：允许通过环境变量 FORCE_GPU=1 禁用 CPU offload（也可在 main 中用 args 传入）
    force_gpu = os.environ.get("FORCE_GPU", "0") == "1" and use_cuda
    device_map = {"": torch.cuda.current_device()} if force_gpu else "auto"
    ...
    # 显式设置 pad_token_id（同时更新 tokenizer）
...
def main():
    ...
    if official_tuple is not None:
        ...
        model, processor = init_model_and_processor(model_id, args.device)
